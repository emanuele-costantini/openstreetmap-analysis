...
import logging
import numpy as np
from util import GeoUtil as GU, FileDirNames as DIR
...
class RoadsDataFrame:
    ...
    def graph_df_preproc(nodes: pd.DataFrame, edges: pd.DataFrame) -> pd.DataFrame:
        ...
        roads = (
            edges
            .merge(nodes, left_on="u", right_on="point_osmid", how="left")
    ...
    def df_avg_curveness(df_in: pd.DataFrame, first_method: bool) -> pd.DataFrame:
        ...
        if first_method:
            logging.info("Computing average sinuosity of entire road by name (first method)")
        if not first_method:
            logging.info("Computing average sinuosity of entire road by name (second method)")
        ...
        df_in[col_creation] = df_in.groupby("name")[col_group].transform(
            GU.compute_avg_curveness,
            first_method
        )

        if not first_method:
            df_in["geometry"] = df_in.apply(
                lambda x: wkt.loads(x["geometry"]),
                axis=1
            )
        return df_in
